...
const PengeluaranModal = ({
  ...
  // Tambahan wajib buat ngatur dropdown buka/tutup
  ...
  const handleClose = () => {
    setIsKategoriOpen(false); // Reset dropdown pas modal ditutup
    onClose();
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="relative bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 max-w-md w-full max-h-[90vh] overflow-y-auto"
          >
            <div className="p-8">
              ...
              <form onSubmit={onSubmit} className="space-y-6">
                ...
                {/* INI BAGIAN DROPDOWN YANG UDAH DIROMBAK TOTAL */}
                <div>
                  <label className={labelStyle}>Kategori</label>
                  <div className="relative">
                    {/* Hidden select biar validasi form HTML tetep jalan */}
                    <select
                      name="kategori"
                      value={form.kategori}
                      onChange={handleChange}
                    ...
                    {/* Tombol yang kelihatan di layar */}
                    <button
                      type="button"
                    ...
                    {/* Pop-up list yang melengkung estetik */}
                    <AnimatePresence>
                      {isKategoriOpen && (
                        <motion.div
                          ...
                          <div className="p-2 flex flex-col gap-1">
                            {kategoriOptions.map((option) => (
                              <button
                                ...
                                type="button"
                                onClick={() => {
                                  // Update state form langsung ke nama kategori
                                  setForm({ ...form, kategori: option });
                                  setIsKategoriOpen(false); // Tutup dropdown
                                }}
