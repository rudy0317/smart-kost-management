...
const KamarTable = ({ data, onEdit, onDelete, onSort, renderSortIcon }) => {
  ...
  const seriKeys = Object.keys(groups).sort()
        ...
        <table className="w-full text-left border-collapse">
          <thead className="sticky top-0 z-10 bg-slate-50/80 backdrop-blur-md">
            <tr className="border-b border-slate-100">
              ...
              <th className={thStyle}>Seri</th>
              <th className={thStyle}>Renovasi</th>
          ...
          <tbody className="divide-y divide-slate-50">
            <AnimatePresence mode="popLayout">
              {seriKeys.map((seri) => [
                ...
                >
                  <td colSpan={9} className="px-6 py-3 text-xs font-black text-indigo-600 uppercase tracking-widest">
                    ══ Seri {seri} ══
                  </td>
                </motion.tr>,
                ...groups[seri].map((k) => (
                  <motion.tr
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    className="hover:bg-slate-50/70 group"
                  >
                    <td className="p-6">
