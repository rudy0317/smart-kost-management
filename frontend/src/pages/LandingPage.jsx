    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="relative h-[80vh] flex items-center justify-center bg-gray-900 overflow-hidden">
        ...
        <div className="relative z-10 text-center px-4 max-w-3xl">
          <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-6 tracking-tight">
            Selamat Datang di <span className="text-indigo-400">Kost Async</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="#katalog" className="px-8 py-4 bg-white text-gray-900 font-bold rounded-full hover:bg-gray-100 transition-all shadow-xl">
              Lihat Katalog
            </a>
            <button
              onClick={() => navigate('/pesan')} // Lari ke PemesananPublik.jsx
              className="px-8 py-4 bg-indigo-600 text-white font-bold rounded-full hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/20"
            >
              Pesan Sekarang
            </button>
          </div>
      ...
      <div id="katalog" className="py-20 px-4 md:px-10 max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-3xl font-extrabold text-gray-900 mb-4">Katalog Kamar Kami</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {kamar.length > 0 ? (
            kamar.map((k) => (
              <div key={k.id} className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 group">
                {/* Image Placeholder */}
                <div className="h-56 bg-gray-200 relative overflow-hidden">
                  ...
                  <img
                    src="https://via.placeholder.com/400x300?text=Kost+Mewah"
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-xl font-bold text-gray-900">Kamar {k.nomor}</h3>
                    <p className="text-indigo-600 font-extrabold">Rp {Number(k.harga).toLocaleString('id-ID')}</p>
                  </div>
                  <p className="text-gray-500 text-sm mb-6 line-clamp-2">
                    {k.fasilitas || 'Fasilitas lengkap: WiFi, Kamar Mandi Dalam, Kasur, Lemari.'}
                  </p>
                  <button
                    onClick={() => navigate('/pesan')} // Langsung lari ke form booking
